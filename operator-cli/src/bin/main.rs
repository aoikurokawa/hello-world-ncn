...
async fn main() -> anyhow::Result<()> {
    ...
    let handler = Handler::new(&args.rpc_url, &payer, args.program_id, args.priority_fees);
    ...
    loop {
        ...
        info!("Checking for message. Slot: {slot}, Current Epoch: {epoch}");
        ...
        //     })
        //     .collect();
        if let Ok(msg) = handler.get_message(epoch).await {
            let keyword = String::from_utf8_lossy(&msg.keyword[..msg.keyword_len as usize]);

            info!("Got Message: {keyword}");

            let message = format!("{keyword}, World");
            handler
                .submit_message(&args.ncn_pubkey, &args.operator_pubkey, epoch, message)
                .await
                .unwrap();
        }
